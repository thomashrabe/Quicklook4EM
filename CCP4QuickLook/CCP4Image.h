...
typedef struct {
	...
	uint32_t lskflg;//Flag for skew transformation, =0 none, =1 if foll
	uint32_t skwmat[9]; // Skew matrix S (in order S11, S12, S13, S21 etc) if LSKFLG .ne. 0.
	uint32_t skwtrn[3]; //Skew translation t if LSKFLG .ne. 0.
	uint32_t future[15];//future use
	uint32_t map; //Character string 'MAP ' to identify file type
...
@interface CCP4Image: NSObject{
	CCP4_Header fileHeader;
	CGImageRef theImageRef;
}

@property (assign) CCP4_Header fileHeader;
@property (assign) CGImageRef theImageRef;

+ (CCP4Image*) CCP4DataFromFile:(NSString*) filename;
